...
export const ToolbarTextInput = ({onChange,value,prefix,label,type,...props}) => {
  ...
  return (
    <div style={{ width: '100%', position: 'relative'}} onClick={() => {setActive(true); }} >
      {(type == 'color' || type == 'bg') && active ? (
        <div className="absolute" style={{zIndex: 99999,top: 'calc(100% + 10px)',left: '-5%'}}>
          <div className="fixed top-0 kunta left-0 w-full h-full cursor-pointer" onClick={(e) => {e.preventDefault();e.stopPropagation();setActive(false);}}></div>
          <ChromePicker
            color={value}
            onChange={(color) => {
              onChange(color.rgb);
